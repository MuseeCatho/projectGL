...
function description_modifie(){
    ...
    var count=0;
    var gap=0;// decalage du texte modifiï¿½
        ...
        if(list_text[j+gap2]!=list_text_insert[j+gap] && typeof list_text[j+gap2]!="undefined"){
            ...
            while(pos_X==-1){// on devrait executer cette boucle que
                ...
                if(pos_X==-1){// si le mot suivant dans list_text n'existe pas
								// dans list_insert c'est qu'il a ï¿½tï¿½ supprimï¿½
								// donc il faut chercher le mot suivant

                    list_word_delete.push(k+gap2);// on stocke les mots
													// supprimï¿½s
                    if(first==1)// pour une phrase supprimï¿½ on stocke l'indice
								// de son premier mot
                    ...
                    notdoublon=true;
						...
						// possibilitï¿½s, soit le mot trouvï¿½ est en rï¿½alitï¿½ un
						// autre mot et donc le mot en question ï¿½ ï¿½tï¿½ supprimï¿½
                    ...
                    if(list_of_doublon_index_text_modified.length < list_of_doublon_index.length){// il y
                        ...
                        if(first==1)// pour une phrase supprimï¿½ on stocke
                        ...
                        doublon = true;
                        k++;
                        pos_X=-1;// on le met ï¿½ -1 de maniere a rester dans
									// la boucle for
                         
                    }
                    else{// dans ce cas ce n'est pas forcement une non
							// suppression puisque l'utilisateur pourrait
							// inserer du texte plus loin contenant le mï¿½me mot
                        // console.log(list_of_doublon_index_text_modified);
                        // console.log(list_of_doublon_index);
                        notdoublon=true;
                    }
            ...
            if(word_delete!="")// insertion des mots supprimï¿½s
                list_word_delete_string.push(word_delete);
            // ci dessous ï¿½ modifier
            if(!doublon || notdoublon){
                for(var i=j+gap;i<pos_X;i++){// stocke ï¿½ priori les indices
												// des mots qui on ï¿½tï¿½ insï¿½rï¿½s
...
function annulation() {
	var r = confirm("Annuler les modifications et revenir Ã  la page de l'oeuvre?");
	if (r == true) {		
		reset()
	}
}
